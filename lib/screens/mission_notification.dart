...
class MissionNotification extends StatefulWidget {
  final int model;
  const MissionNotification({super.key, required this.model});
...
class _MissionNotificationState extends State<MissionNotification> {
  ...
  Future<void> _fetchMissionDates() async {
    try {
      var sessionDates = await apiConnection.getOneMission(widget.model);
      setState(() {
        mission = sessionDates;
        isLoading = false;
      });
              ...
              child: Column(
                children: [
                  ...
                  isLoading
                      ...
                      : Expanded(
                          child: DateList(
                            startDate: mission.sessionStartDates,
                            endDate: mission.sessionEndDates,
                            itemCount: mission.sessionStartDates.length,
                          ),
                        ),
                  const SizedBox(
                    height: 16,
                      ...
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (context) => DateDetails(
                                  startDate: mission.sessionStartDates,
                                  endDate: mission.sessionEndDates,
                                  itemcount: mission.sessionEndDates.length)));
                    },
